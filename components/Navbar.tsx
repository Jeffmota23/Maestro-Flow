        ...
        <div className="flex justify-between items-center h-20">
          ...
          <div className="flex items-center space-x-4 md:space-x-8">
            ...
            <div className="relative">
              ...
              {showLangMenu && (
                <div className="absolute right-0 mt-3 w-56 bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl p-2 z-[100] animate-in fade-in zoom-in-95 duration-200">
                  {languages.map((lang) => (
                    <button
                      key={lang.code}
                      onClick={() => {
            ...
            <button 
              ...
              className="relative p-2.5 text-slate-300 hover:text-indigo-400 transition-colors bg-slate-800 rounded-xl"
            >
              <ShoppingCart className="w-5 h-5" />
              {items.length > 0 && (
                <span className="absolute -top-1 -right-1 flex h-5 w-5 items-center justify-center rounded-full bg-indigo-600 text-[10px] font-black text-white shadow-lg ring-2 ring-slate-900">
                  {items.length}
                </span>
              )}
            </button>
            ...
            {user ? (
              <div className="flex items-center space-x-4">
                <button 
                  ...
                  className="flex items-center space-x-2 text-sm font-bold text-slate-200 hover:text-indigo-400 bg-slate-800 px-4 py-2 rounded-xl transition-all"
                >
              ...
              <button 
                ...
                className="px-6 py-2.5 text-sm font-bold text-white bg-indigo-600 rounded-xl hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-600/20"
              >
                {t('signIn')}
              </button>
            )}
